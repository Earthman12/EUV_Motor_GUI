...
class EUVGUI(object):
    ...
    def __init__(self):
        ...
        # Moved the self.command.get() into the sendCommand so you can populate the string with each
        # "Send" press 
        self.commandButton = tk.Button(text = "Send", command = lambda :[self.OmsLib.sendCommand(str(self.command.get())), self.updateLog()])
...
class Oms(object):
    ...
    def __init__(self, gui):
        ...
        self.lib = ct.WinDLL("C:\\Users\\ualpl\\Desktop\\EUV_Motor_GUI\\Important_Files\\Win7\\x64\\OmsMAXkMC.dll")
        print("lib: ", self.lib)

        #OMS HANDLE, THIS MAY NEED TO BE CHANGED DEPENDING ON CARD SLOT

        str_buff = [ct.create_string_buffer(b"OmsMAXk1")]
        str_buff_pt = (ct.c_char_p)(*map(ct.addressof, str_buff))

        self.handle = self.lib.GetOmsHandle(str_buff_pt)
...
#--------------------------------------------------------------------------------------------------
    ...
    def sendCommand(self, arg):
        ...
        # c_arg = ct.c_wchar_p(arg)
        # cmd_buff = ct.create_string_buffer(80)

        # c_returnStatus = self.lib.SendString(self.handle, c_arg)

        ### New ###
        # arg_ptr = ct.c_wchar_p(arg)
        print("raw arg:", arg)
        arg_buff = ct.create_string_buffer(bytes(arg,'utf-8'))
        # arg_buff_ptr = (ct.c_char_p(arg_buff))

        print("arg: ", arg_buff.value)
        c_returnStatus = self.lib.SendString(self.handle, arg_buff)
        ### New ###
